...
- Databases: Organized collection of structured information/data that is easily accessible and can be manipulated or analyzed. Example: User authentication data, user generated data, user history data.
	...
	- Non-relational Databases: (NoSQL): Store data in a non-tabular format. Such as storing data that can contain varying types of quantities of data. Better used when data received can vary greatly in it's format, such as user-generated content.
		- Reference Image: [[TryHackMe/Pasted image 20250406085215.png|Non-Relational Databases]]
- Relational Databases In-Depth:
	- Reference Image: [[TryHackMe/Pasted image 20250406085528.png|Tables, Rows, and Columns]]
	...
	- Primary and Foreign Keys:
		- Reference Image: [[TryHackMe/Pasted image 20250406090026.png|Primary and Foreign Keys]]
	...
	- Commands:
		...
		- Table Statements:
			- `CREATE TABLE example_table_name`: Create a table
				- Reference Image: [[TryHackMe/Pasted image 20250406091453.png|CREATE TABLE Syntax]] and [[TryHackMe/Pasted image 20250406091824.png|CREATE TABLE Usage]] 
		...
		- CRUD Operations: 
			...
			- Create Operation (INSERT): 
				- Usage: `INSERT INTO`
				- Reference Image: [[TryHackMe/Pasted image 20250406092647.png|INSERT INTO Example]]
			- Read Operation (SELECT):
				- Usage: `SELECT * FROM table_name;`
				- Another example: `SELECT name, description FROM table_name;`
			- Update Operation (UPDATE): 
				- Usage: `UPDATE`
				- Reference Image: [[TryHackMe/Pasted image 20250406093151.png|UPDATE Example]]
			- Delete Operation (DELETE):
				- Usage: `DELETE`
				- Reference Image: [[TryHackMe/Pasted image 20250406093339.png|DELETE Example]]
		- Clauses: 
			...
			- `HAVING`: Used with order clauses to filter groups or results of records based on a condition
				- Usage: [[TryHackMe/Pasted image 20250406110654.png|HAVING Clause Example]]
		...
		- Operators: 
			...
			- Logical Operators: Test the truth of a condition and return `TRUE` or `FALSE`
				...
				- `AND`: Uses multiple conditions and returns `TRUE` if all of them are true.
					- Example: [[TryHackMe/Pasted image 20250406111509.png|AND Operator Example]]
				- `OR`: Combines multiple conditions and returns `TRUE` if at least one is true.
					- Example: [[TryHackMe/Pasted image 20250406111740.png|OR Operator Example]]
				- `NOT`: Reverses the value of a Boolean operator to exclude a specific condition.
					- Example: [[TryHackMe/Pasted image 20250406111850.png|NOT Operator Example]]
				- `BETWEEN`: Allows us to test if a value exists within a defined range
					- Example: [[TryHackMe/Pasted image 20250406112000.png|BETWEEN Operator Example]]
		...
		- Functions: 
			...
			- String Functions:
				- `CONCAT()`: Add two or more strings together.
					- Example: [[TryHackMe/Pasted image 20250406113950.png|CONCAT Function Example]]
				- `GROUP_CONCAT()`: Concatenate data from multiple rows into one field.
					- Example: [[TryHackMe/Pasted image 20250406114144.png|GROUP_CONCAT Function Example]]
				- `SUBSTRING()`: Retrieve a substring from a string, starting at a determined position.
					- Example: [[TryHackMe/Pasted image 20250406114337.png|SUBSTRING Function Example]]
