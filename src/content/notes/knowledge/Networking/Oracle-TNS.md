...
category: "knowledge"
tags: []
excerpt: "Oracle TNS (Transparent Network Substrate) is Oracle's proprietary communication protocol for connecting Oracle..."
date: "2026-06-20"
---
...
Scan port 1521 with [Nmap](/tools/Enumeration/Nmap) (`-sV --open`). The `oracle-sid-brute` NSE script brute-forces valid SID names from a wordlist. See [Nmap](/tools/Enumeration/Nmap) for full script reference.

Perform a full enumeration scan (SIDs, versions, users, vulnerabilities) with [ODAT](/tools/Enumeration/ODAT).

---

### Connecting with SQLplus

Connect using the `sqlplus <user>/<password>@<target>/<service-name>` syntax. Append `as sysdba` if the account holds the sysdba privilege. See [SQLplus](/tools/Utilities/SQLplus) for installation and full connection syntax.
...
For installation steps and library dependency fixes, see [SQLplus](/tools/Utilities/SQLplus).

---

### Webshell Upload via ODAT
If a web server runs on the same host, the `utlfile` module in [ODAT](/tools/Enumeration/ODAT) can write files directly to the web root (`/var/www/html` on Linux, `C:\inetpub\wwwroot` on Windows). See [ODAT](/tools/Enumeration/ODAT) for the full webshell upload workflow.
...
## Related Tools
- [ODAT](/tools/Enumeration/ODAT)
- [SQLplus](/tools/Utilities/SQLplus)
