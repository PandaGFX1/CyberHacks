...
category: "knowledge"
tags: []
excerpt: "IPMI (Intelligent Platform Management Interface) is a set of standardized specifications for hardware-based host..."
date: "2026-06-20"
---
...
HP iLO uses a factory-generated password — an 8-character string of uppercase letters and digits. If the RAKP hash is retrieved, it can be brute-forced offline with [Hashcat](/tools/Password-Attacks/Hashcat) using mode 7300 against a mask covering all combinations of 8 uppercase letters and numbers.
...
Scan UDP/623 with [Nmap](/tools/Enumeration/Nmap) using the `ipmi-version` NSE script to confirm the IPMI service and firmware version.

Identify IPMI version and dump RAKP password hashes using the `ipmi_version` and `ipmi_dumphashes` auxiliary modules in [MSFConsole](/tools/Frameworks/Metasploit/MSFConsole).

Crack RAKP hashes offline with [Hashcat](/tools/Password-Attacks/Hashcat) (RAKP mode) or [JohnTheRipper](/tools/Password-Attacks/JohnTheRipper) (RAKP format). No server access required — the hash is obtainable from any valid username due to the protocol-level RAKP flaw.
...
- [Footprinting & Enumeration](/knowledge/Offensive-Security/Footprinting-Enumeration)
...
- [Hashcat](/tools/Password-Attacks/Hashcat)
- [JohnTheRipper](/tools/Password-Attacks/JohnTheRipper)
- [MSFConsole](/tools/Frameworks/Metasploit/MSFConsole)
