...
category: "knowledge"
tags: []
excerpt: "Windows provides three primary remote management protocols: RDP for graphical desktop access, WinRM for command-line..."
date: "2026-06-20"
---
...
| xfreerdp | Linux RDP client — see [xfreerdp](/tools/Utilities/xfreerdp) |
...
Scan TCP/3389 with [Nmap](/tools/Enumeration/Nmap) (`rdp*` NSE scripts) to fingerprint the service and enumerate security settings. Check for weak encryption and supported security protocols (RDP Security, TLS, NLA) with [rdp-sec-check](/tools/Enumeration/rdp-sec-check).

Connect from Linux via [xfreerdp](/tools/Utilities/xfreerdp).
...
Scan TCP/5985 and TCP/5986 with [Nmap](/tools/Enumeration/Nmap) to confirm WinRM is listening. Connect from Linux using [evil-winrm](/tools/Post-Exploitation/evil-winrm).
...
Execute WMI commands remotely from Linux using `wmiexec.py` (part of Impacket). For full `Get-WmiObject` and `Get-CimInstance` query reference, see [Windows PowerShell](/knowledge/Operating-Systems/Windows/Windows-PowerShell).
...
- [rdp-sec-check](/tools/Enumeration/rdp-sec-check)
